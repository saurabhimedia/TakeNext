...
namespace gv.ampp.control.demo.app
{
    public class RollingLogger : IDisposable
    {
        ...
        public void log(string message) =>
            _queue.TryAdd($"{DateTime.Now:yyyy-MM-dd'T'HH:mm:ss.fff}-{GetThreadTag()} : {message}");
        ...
        {
            // Prefer a friendly name; fallback to ManagedThreadId
            var name = Thread.CurrentThread.Name;
            var id = Thread.CurrentThread.ManagedThreadId;
            return !string.IsNullOrEmpty(name) ? $"{name}/{id}" : $"T{id}";
        }
