...
namespace gv.ampp.control.demo.app
{
    ...
    public class CrashLiveApp
    {
        ...
        private static int TAKE_STARTTIME_DELAY = 500;
       ...
       /// <summary>
        ...
        /*public CrashLiveApp(String appName, DemoConfiguration configuration, AmppControlService amppControlService,
            String platformUrl, String apiKey, String ip, short port, String networkId,Dictionary<String,String> props)
        {
            ...
            refreshThread.Start();
            this.appCmdMap = props;
            String val = appCmdMap["TIME_BETWEEN_COMMANDS"];
            if (val != null)
            {
            ...
            logTimeStamp = new LogUtil("GoLiveTimeStamp.txt", FileMode.Create);
            ...
            String platformUrl, String apiKey, Dictionary<String, String> props)
        {
            ...
            if (val!=null && val.ToLower().Equals("yes") )// only for RossTalk
            {
        ...
        public async Task sendIPTrigger(JObject payload, string reconKey)
        {
            try
            {
                ...
                try
                {
                    if (cs == null)
                    {
                        log.log("Trying to establish a connection to "+ip+ ":" + port);
                ...
                log.log("Command to Send is " +rsCmd);
                //String command = rsCmd;
                string command=CommonUtils.MakeProperCommandString(rsCmd); //-- Santosh Commented, mostly for Vizart required
                log.log("Final Command to Send after encoding is " + command + " to " + cs);
               // cs.SendIPCmdAsync(command, DELAY_AFTER_CMD);
                _ = Task.Run(async () =>
                {
                    try
                    {
                        await cs.SendIPCmdAsync(command, DELAY_AFTER_CMD);
                    }
                    catch (Exception ex)
                    {
                        log.log("SendIPCmdAsync (bg) failed: " + ex);
                    }
                });
                Thread.Sleep(1000);
            ...
            if (!nextItemId.Equals(""))
            {
                ...
                { 
                    if (autoCueReadyItem)
                    {
                        ...
                        if (await this.SendCueCommandAsync(runDownId, nextItemId))
                            status = "Cued";
                    }
                    else
                    {
        ...
        // get Next
        //https://apac1.gvampp.com/orchestration/api/v1/schedules/xl03-plx-01/next?itemsToReturn=1&sameParent=false
        /*
        private async Task<String> getNextID(String rundownID)
        {
            String nextItemId = "",status="";
            try
            {
                String url = "https://apac1.gvampp.com/orchestration/api/v1/schedules/" + rundownID + "/next?itemsToReturn=1&sameParent=false";
                using HttpClient client = new HttpClient();
                client.BaseAddress = new Uri(url);
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", bearerToken);
                log.log("Sent request to Query next Item to take for networkID:" + rundownID);
                HttpResponseMessage response = await client.GetAsync(url);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    try
                    {
                        ...
                        foreach (JObject innerItem in innerItemsArray)
                        {
                            ...
                            JObject state = (JObject)((JArray)innerItem["channelItemStates"])[0];
