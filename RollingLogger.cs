    ...
    public class RollingLogger : IDisposable
    {
        ...
        public getInstance()
        {
            try
            {
                ...
                {
                    createLogDir();
                    inst = new RollingLogger("TakeNextLog.txt", 5);
                }
                return inst;
            }
        ...
        public void WriteLog(string message)
        {
            ...
            _currentLogWriter.WriteLine($"{DateTime.Now:yyyy-MM-dd HH:mm:ss} - {message}");
        ...
        {
            // Example usage:
            using (RollingLogFile logger = new RollingLogFile(@"C:\Logs", "appLog", 5)) // 5 MB max size for each log file
            {
                // Write some log entries
                for (int i = 0; i < 20; i++) // Write 20 log entries to test the rolling
                {
                    logger.WriteLog($"Log Entry {i + 1}");
                }
